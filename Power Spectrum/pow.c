...
float func (float k)
{ 
    float window = abs(3/pow((8*k),3) * (sin(8*k) - 8*k*cos(8*k))) ;
    float tk = pow(TFmdm_onek_hmpc(k),2) * pow(k , 0.968);
    return pow(k,2)/(2*pow(M_PI,2)) * tk * pow(window,2) ;   
}
...
int main ()
{
	...
	float tk[1000] ; // tkcdm[1000], tkplot[1000];
	float Ah , sigma8 = 0.8 ;

	TFmdm_set_cosm (1, 0.05, 0.2, 1, 0, 0.5, 0) ;//hubble = 0.69
	for (int i = 0 ; i < div ; i++)
	{
		tk[i] = TFmdm_onek_hmpc(pow(10 , start + i*step)) ;
		printf ("%f %f\n" , pow(10, start + i*step) , TFmdm_onek_hmpc(pow(10, start + i*step))) ;
	}

	Ah = pow(sigma8,2)/integrate(0,250,5000) ;

	/*
	TFmdm_set_cosm (1, 0, 0, 1, 0, 0.5, 0) ;
	for (int i = 0 ; i < div ; i++)
	{
		tkcdm[i] = TFmdm_onek_hmpc(pow(10 , start + i*step)) ;
		tkplot[i] = tk[i]/tkcdm[i] ;
		printf ("%f %f\n" , start + i*step , tkplot[i]) ;
	}
	*/
