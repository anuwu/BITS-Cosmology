...
double   tf_cb,		/* The transfer function for density-weighted
	...
	tf_cbnu;	/* The transfer function for density-
...
// Trial 1 (0.3, 0.03, 0.06, 1, 0.7, 0.8, 0) divided by (0.21, 0, 0, 1, 0.79, 0.8, 0)
// Trial 2 (1, 0.05, 0.2, 1, 0, 0.5, 0) divided by (1, 0, 0, 1, 0, 0.5, 0)
// Power spectrum (0.308, 0.05, 0, 1, 0.692, 0.69, 0)
int TFmdm_set_cosm (double omega_matter, double omega_baryon, double omega_hdm,
	int degen_hdm, double omega_lambda, double hubble, double redshift)
...
{ 
	...
	if (k == 0)
    	...
    	window = 3/pow((8*k),3) * (sin(8*k) - 8*k*cos(8*k)) ;

    double tk = pow(TFmdm_onek_hmpc(k),2) * pow(k , ns);
    double val = pow(k,2)/(2*pow(3.141593,2)) * tk * pow(window,2) ;  
...
int main ()
{
	...
	double tk[div], tkcdm[div], tkplot[div];
	double Ah , sigma8 = 0.8 ;
	double k ;

	
	//(double omega_matter, double omega_baryon, double omega_hdm, int degen_hdm, double omega_lambda, double hubble, double redshift)
	
	//TFmdm_set_cosm (1, 0.05, 0.20, 1, 0, 0.5, 0) ;		//fib 1a
	//TFmdm_set_cosm (0.3, 0.03, 0.06, 1, 0.7, 0.5, 0) ;	//fig 2b
	//TFmdm_set_cosm (1, 0.30, 0.20, 1, 0, 0.5, 0)	;		//fig 2a
	for (int i = 0 ; i < div ; i++)
	{
		k = 0.001 * pow(10, start + i*step) ; 
		tk[i] = TFmdm_onek_hmpc(k) ;
		//printf ("%f %f %f\n" , pow(10, k) , TFmdm_onek_hmpc(pow(10, k)) , pow(TFmdm_onek_hmpc(pow(10, k)),2)) ;
	}
	
	//TFmdm_set_cosm (1, 0, 0, 1, 0, 0.5, 0) ;			//fib 1a, 2a
	//TFmdm_set_cosm (0.3, 0, 0, 1, 0.7, 0.5, 0) ;		//fig 2b
	for (int i = 0 ; i < div ; i++)
	{
		k = 0.001 * pow(10, start + i*step) ;
		tkcdm[i] = TFmdm_onek_hmpc(k) ;
		tkplot[i] = tk[i]/tkcdm[i] ;
		printf ("%f %f\n" , k , tkplot[i]) ;
	}

	/*
	
	TFmdm_set_cosm (0.308, 0.05, 0, 1, 0, 0.69, 0) ;
    Ah = pow(sigma8,2)/(integrate(0,3,5000) + integrate(3,10,5000) + integrate(10,100,5000));
	printf ("Ah = %.17g\n" , Ah) ;
	...
	for (int i = 0 ; i < div ; i++)
	{
	    k = start + i*step ;  
		printf ("%.17g %.17g\n" , k,
		log10(Ah * pow(TFmdm_onek_hmpc(pow(10, k)),2) * (pow(pow(10, k),2) , ns))) ;
	}
	*/
